...
def main():
    ...
    # clicker_amount = 100000000000000000000000**10
    # tps = 10**138
    LOGGER.DEBUG(
    ...
    if last_saved_time:
        print(datetime.now())
        print(datetime.strptime(last_saved_time, " %Y-%m-%d %H:%M:%S"))
        elapsed_time = (
    ...
    timeUnits_text: pg.Surface = get_number_font(65, he).render(
        f"{format_timeUnits(timeUnits, 0)}", True, RED_OCHRE
    )
    ...
    tps_text: pg.Surface = get_number_font(50, he).render(
        f"{format_timeUnits(tps, 0)}", True, RED_OCHRE
    )
    ...
    while running:
        ...
        timeUnits_text_logo: pg.Surface = load_image(
            f"{src_dir}/img/timeUnits.png", w, h
        )

        # tps = 17.4**138
        tps_text: pg.Surface = get_number_font(40, h).render(
            f"{format_timeUnits(tps, 0)} ", True, RED_OCHRE
        )
        tps_text_logo: pg.Surface = load_image(
            f"{src_dir}/img/tps.png", w, h, scale=0.9
        )
        ...
        for event in pg.event.get():
            ...
            elif (keys[pg.K_LCTRL] or keys[pg.K_RCTRL]) and keys[K_F1]:
                save_data(appdata_path)
                show_message(f"The game has been reseted, please restart the game.")
                running = False
            ...
            if not can_buy_buildings(
                ...
                screen.blit(
                    ...
                    (
                        ...
                        adapt_size_height(132.5 + 105 * i, h)
                        + adapt_size_height((scroll_y * 1), h),
                    ),
                )
        ...
        for i in range(len(upgrades_buttons)):
            ...
            if not can_buy_upgrade(
                ...
                screen.blit(
                    get_text_font(17.5, h).render(
                        f"{format_timeUnits(round(cost))}", True, GREY
                    ),
                    get_text_font(17.5, h)
                    .render(f"{format_timeUnits(round(cost))}", True, GREY)
                    .get_rect(
                        center=(adapt_size_width(x, w), adapt_size_height(y + 32.5, h))
                    ),
                )
                ...
                if not upgrade_level == len(treshold):
                    screen.blit(
                        ...
                        .get_rect(
                            center=(
                                adapt_size_width(x, w),
                                adapt_size_height(y + 32.5, h),
                            )
                        ),
                    )
                else:
                    screen.blit(
                        get_text_font(17.5, h).render(f"{cost}", True, WHITE),
                        get_text_font(17.5, h)
        ...
        screen.blit(
            tps_text,
            tps_text.get_rect(
                center=(adapt_size_width(860, w), adapt_size_height(212.5, h))
            ),
        )
        screen.blit(
            tps_text_logo,
            tps_text_logo.get_rect(
                midleft=tps_text.get_rect(
                    center=(adapt_size_width(860, w), adapt_size_height(212.5, h))
                )
                .move(0, adapt_size_height(-5, h))
                .midright
